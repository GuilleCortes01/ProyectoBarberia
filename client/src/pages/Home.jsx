      ...
      <section className="relative min-h-[calc(100vh-5rem)] overflow-hidden">
        ...
        <div className="container-page relative flex min-h-[calc(100vh-5rem)] items-center py-16">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-4xl">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-gold/30 bg-gold/10 px-4 py-2 text-sm font-bold text-gold"><Star size={16} /> Barberia premium con turnos online</div>
            <h1 className="font-display text-5xl font-extrabold leading-tight sm:text-7xl">Urban Barber Studio</h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-200">Cortes precisos, fades impecables y una experiencia moderna para clientes que quieren reservar rapido y salir con estilo.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              {isAdmin ? (
                <Link to="/admin" className="btn-primary"><CalendarDays size={18} /> Ir al panel</Link>
              ) : (
                <Link to="/reservar" className="btn-primary"><CalendarDays size={18} /> Reservar turno</Link>
              )}
              <a href="https://wa.me/5493815551234" target="_blank" rel="noreferrer" className="btn-secondary"><MessageCircle size={18} /> WhatsApp</a>
