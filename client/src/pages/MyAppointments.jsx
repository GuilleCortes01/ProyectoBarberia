    ...
    <section className="container-page py-16">
      ...
      <div className="grid gap-4">
        {appointments.map((item) => (
          <article key={item.id} className="glass-panel grid gap-4 rounded-lg p-5 md:grid-cols-[1fr_auto] md:items-center">
            <div>
            ...
            <div className="flex flex-col gap-2 sm:flex-row md:flex-col">
              <a className="btn-secondary py-2" href={whatsappUrl(business?.phone, appointmentMessage(item, "Hola, queria consultar por mi turno"))} target="_blank" rel="noreferrer"><MessageCircle size={16} /> WhatsApp</a>
              {!["CANCELLED", "FINISHED"].includes(item.status) && <button className="btn-secondary py-2" onClick={() => cancel(item.id)}>Cancelar</button>}
              {item.status === "CANCELLED" && <button className="btn-secondary py-2" onClick={() => remove(item.id)}>Eliminar</button>}
            </div>
          </article>
        ))}
